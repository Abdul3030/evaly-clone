...
const Slider2 = () => {
    ...
    return (
        <Swiper navigation loop autoplay={{delay: 3000}} slidesPerView={2} spaceBetween={10} >
            ...
            {
                images.map((item, idx) => (
                <SwiperSlide key={idx} className="mt-2">
                    <div className="w-full h-60 text-white flex justify-center items-center bg-gradient-to-r from-red-300 via-red-700 to-red-300">
                        <Link href={`${idx}`}>
                            <a >
                                <img className='h-60 min-w-full' src={item.src} alt="Slide"/>
                            </a>
                        </Link>
                    </div>
                </SwiperSlide>))
